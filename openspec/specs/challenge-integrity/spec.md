...
## Purpose
Keep the CTF-2 LFI-to-RCE Docker-in-Docker challenge honest and reproducible: the walkthrough documents the real LFI direct-read shortcuts, the offline climax reuses a locally-present image, and the inner sshd host keys are deterministic so the intended solve path is always reachable.
